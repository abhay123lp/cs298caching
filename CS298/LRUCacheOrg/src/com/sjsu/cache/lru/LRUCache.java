	...
	private void addItem(Request theRequestedItem)
	{
		...
		switch (caseId) 
		{
			case 1:	// Case 1: Cache is Full && Item to be cached is duplicated ---> Cache Hit
				...
				totalCacheHitSize = totalCacheHitSize + theRequestedItem.getRequestedItemSize();
			...
			case 2: // Case 2 : Cache is Full && Item to be cached is not duplicated ---> Cache Miss
				...
				cacheMap.put(requestedItem, new Long(theRequestedItem.getRequestedItemSize()));	//4) Cache the new Item in the LRUQueue
				totalCacheMissSize = totalCacheMissSize + theRequestedItem.getRequestedItemSize();
			...
			case 4: //Case 4: Cache is not Full && Item to be cached is not duplicated ---> Cache Miss
				...
				cacheMap.put(requestedItem, new Long(theRequestedItem.getRequestedItemSize()));		//2) cache the new Item in the cacheMap
				totalCacheMissSize = totalCacheMissSize + theRequestedItem.getRequestedItemSize();
				cacheMiss+=CacheConstants.ONE;
				break;
				
			case 0: // default casecomputeCase
				System.out.println("Case 0 : LRUCache 98 -> addItem()");
				break;
	
			default: //default casecomputeCase
