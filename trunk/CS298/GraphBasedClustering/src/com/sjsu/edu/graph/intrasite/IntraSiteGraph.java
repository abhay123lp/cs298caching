package com.sjsu.edu.graph.intrasite;
...
public class IntraSiteGraph {
	...
	public IntraSiteGraph()
	{
		prevNode = null;
		nodeSet = new HashMap<String, Node>();
		directedGraph = new DirectedSparseGraph<Node, Edge>();
	}
	...
	{
		Collection<Node> nodes = directedGraph.getVertices();
	...
	public void addNode(Node nextNode)
	{
		...
		if(prevNode!=null) //Graph is Already created
		{
			...
			if(nodeSet.containsKey(URL))
			{
				...
				edge = directedGraph.findEdge(prevNode,nextNode);
				...
				if(edge!= null)
				{
					...
					e = directedGraph.findEdge(prevNode, nextNode);
				...
				else // Add New Edge
				{
					...
					directedGraph.addEdge(e, prevNode, nextNode);
			...
			else
			{
				...
				directedGraph.addEdge(edge, prevNode, nextNode);
	...
	public void cutWithConfidence(Double confidenceThreshold)
	{
		...
		Collection<Edge> edgeSet = directedGraph.getEdges();
		...
		if(edgeToBeRemoved.size()>0)
		{
			...
			{	
				directedGraph.removeEdge(discardEdge);
			}
		}
	}
	...
	{
		Collection<Node> verticesOfGraph = directedGraph.getVertices();
	...
	{
		Collection<Node> nodesOfGraph = directedGraph.getVertices();
		
		Set<Node> nodesToBeRemoved = new HashSet<Node>();
		
		for (Node node : nodesOfGraph)
		{						
			if(directedGraph.getNeighbors(node).size()== 0)
			{
				nodesToBeRemoved.add(node);
			}
			else if(directedGraph.getNeighborCount(node)== 1)
			{
				Collection<Node> neighbors = directedGraph.getNeighbors(node);
		...
		if(nodesToBeRemoved.size() > 0)
		{
			...
			{
				directedGraph.removeVertex(node);
			}
		}
	}
	...
	{		
		Collection<Node> neighbourNodes = directedGraph.getNeighbors(childNode);
	 ...
	 * @return
	 */
	public Set<Node> processGraph(Double confidenceThreshold,Integer supportThreshold)
	{
		Set<Node> clusterOfNodes = null;
		
		cutWithConfidence(confidenceThreshold);
		
		cutWithSupport(supportThreshold);
		
		clusterOfNodes = removeAllUnconnectedNodes();
	...
	/**
	 * Removes all Unconnected Nodes from 
	 * the Graph
	 * 
	 * @return
	 */
	private Set<Node> removeAllUnconnectedNodes()
	{
		Collection<Node> nodesOfGraph = directedGraph.getVertices();
		Set<Node> intraSiteCluster = new HashSet<Node>();
		
		for (Node node : nodesOfGraph)
		{
			
		 Collection<Node> nodes = directedGraph.getNeighbors(node);
		 	
		 if(nodes.size() > 0)
			 intraSiteCluster.add(node);
			
		}
		
		return intraSiteCluster;
	}
	...
	{		
		Collection<Node> neighbourNodes = directedGraph.getNeighbors(node);
	...
	{		
		Collection<Node> neighbourNodes = directedGraph.getVertices();
